<h1 align="center">Rinzler for Android TV</h1>
...
Rinzler for Android TV builds on the solid foundation of Jellyfin with targeted improvements for TV viewing:
...
Rinzler is the first Android TV client with native Jellyseerr and Seerr support.
...
- **Rinzler Proxy Mode** — route all Jellyseerr/Seerr requests through the Rinzler server plugin (no direct connection needed)  
...
> **Deprecation Notice:** In future versions, the legacy authentication methods (Jellyfin auth and local account login from within the app) will be removed. All Jellyseerr/Seerr connections will be managed exclusively through the Rinzler server plugin.

### Plugin Sync
- **Bidirectional Settings Sync** — sync preferences between the app and the Rinzler server plugin
...
- **Server-hosted rating icons** — icons served from the Rinzler plugin (no bundled assets needed)
...
- **OTA Update System** - Automatic check for new Rinzler versions with in-app update notifications
...
1. Install the **Rinzler server plugin** on your Jellyfin server and configure Jellyseerr/Seerr in the plugin settings
2. In Rinzler, go to **Settings → Plugin** and enable **Plugin Sync**
...
2. In Rinzler, go to **Settings → Plugin → Jellyseerr**
...
We welcome contributions to Rinzler for Android TV!
...
Translations contributed to Rinzler that are universally applicable will be submitted upstream to benefit the entire community.
...
Rinzler for Android TV is built upon the excellent work of:
...
- **Rinzler Contributors** - Everyone who has contributed to this fork
